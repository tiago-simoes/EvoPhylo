...
Data <- DataDF %>%  mutate_all(as.factor) #multistates treated as categorical variables for char distances
#or
Data <- DataDF %>% mutate_all(as.numeric) #multistates treated as numeric variables for char distances



############# DISTANCE MATRIX (Gower distances)  ####################
...
write.csv(Dmatrix, file="Test_Opt2_Dmatrix.csv")



############ Cluster analysis using partitioning around medoids (PAM)
### Use Dmatrix above or import. If importing:

Dmatrix <- read.csv("Dmatrix.csv", header=TRUE, row.names = 1)
Dmatrix <- as.matrix(Dmatrix)


################ Part 1: k selection using silhouette index
#### Find best number of partitions (k)
sil_width <- c(NA)
...
############## Part 2: Clustering with PAM under the best K value from part 1
...
# Main output: Write clusters as character partitions in NEXUS format
...
#(second clustering approach--used to double check results from main output above and provide a better graphic visualization of the clusters)
