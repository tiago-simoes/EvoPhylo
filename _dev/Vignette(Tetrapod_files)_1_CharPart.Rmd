...
### 1.Generate a Gower distance matrix with `get_gower_dist`:
...
### 2. Estimate optimal number of partitions 
...
4.2. Analyze clusters with PAM under chosen k value (from Si) with `make_clusters`.
4.3. Produce a graphic clustering (tSNEs), coloring data points according to PAM clusters, to independently verify PAM clustering. This is set with the `tsne = TRUE` argument within `make_clusters`.
4.4. Exports clusters/partitions to Nexus file with `cluster_to_nexus`.
...
clusters2 <- make_clusters(Dmatrix, k = 3, tsne = TRUE, tsne_dim = 3)
