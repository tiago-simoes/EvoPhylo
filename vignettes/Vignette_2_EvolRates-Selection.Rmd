...
```{r, setup, echo=FALSE}
knitr::opts_knit$set(root.dir = "E:/Git/EvoPhylo/")
...
First, import a Bayesian clock tree using treeio's function `read.mrbayes` (= `read.beast`), or using one of the example trees provided.
...
RateTable_Medians_no_clades <- get_clockrate_table(tree3p, summary = "median")
RateTable_Means_no_clades <- get_clockrate_table(tree3p, summary = "mean")
```
...
tree_nodes<-ggtree::ggtree(tree3p, layout = "rectangular", ladderize=TRUE, right=TRUE, 
...
```{r}
## Import rate table with clade membership (new "clade" column added) from your local directory
rate_table_clades_medians<- read.csv("examples/MultiClockTree/RateTable_Medians_Clades.csv", header = TRUE)

#OR from example data file
data(rate_table_clades_medians)

head(rate_table_clades_medians)

## Get summary statistics table for each clade by clock 
clockrate_summary(rate_table_clades_medians, "Sum_RateTable_Medians.csv", digits=3)
```

```{r, echo = FALSE}
t1<-clockrate_summary(rate_table_clades_medians, digits=2) 
...
## Overlapping plots
clockrate_dens_plot(rate_table_clades_medians, stack = FALSE, nrow = 1, scales = "fixed")
```
...
```{r, fig.width=8, fig.height=5, fig.align = "center", out.width = "100%"}
## Stacked plots
clockrate_dens_plot(rate_table_clades_medians, stack = TRUE, nrow = 1, scales = "fixed")
```
...
p12 <- clockrate_reg_plot(rate_table_clades_medians, clock_x = 1, clock_y = 2)
p13 <- clockrate_reg_plot(rate_table_clades_medians, clock_x = 1, clock_y = 3)
p23 <- clockrate_reg_plot(rate_table_clades_medians, clock_x = 2, clock_y = 3)
...
```{r}
## Import summary tree with a single clock partitions produced by Mr. Bayes (.t or .tre files) from examples directory
tree1p<-treeio::read.mrbayes("examples/SingleClockTree/Tree1p.t")
#OR
data(tree1p)

## Get table of clock rates with summary stats for each node in the tree for each relaxed clock partition 
RateTable_Medians_no_clades <- get_clockrate_table(tree1p, summary = "median")

## Export the rate tables
write.csv(RateTable_Medians_no_clades, file="RateTable_Medians.csv")

## Import rate table with clade membership (new "clade" column added)
rate_table_clades_medians<- read.csv("examples/SingleClockTree/RateTable_Medians_Clades.csv", header = TRUE)

## Get summary statistics table for each clade by clock 
clockrate_summary(rate_table_clades_medians, file="Sum_RateTable_Medians.csv", digits=3)
```
...
```{r}
## Import rate table with clade membership (new "clade" column added) from your local directory with "mean" values
rate_table_clades_means <- read.csv("examples/MultiClockTree/RateTable_Means_Clades.csv", header = TRUE)

#OR from example data file
data(rate_table_clades_means)

## Transform table from wide to long format
RatesByClade <- clock_reshape(rate_table_clades_means)
```
...
RateSign_tests<- get_pwt_rates(rate_table_clades_means, posterior3p)
...
A1<- plot_treerates_sgn(tree3p, posterior3p, 
...
## Plot tree using various thresholds for other clock partition and combine them
A2<-plot_treerates_sgn(tree3p, posterior3p, 
...
A3<-plot_treerates_sgn(tree3p, posterior3p, 
